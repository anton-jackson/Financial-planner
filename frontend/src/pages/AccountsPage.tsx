...
export function AccountsPage() {
  ...
  const addAccount = (type: string) => {
    const defaultNames: Record<string, string> = { ...ACCOUNT_TYPE_LABELS };
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="flex flex-col gap-4 mb-8">
        {investmentAccounts.map(({ asset, index: actualIndex }) => {
              ...
              >
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-medium text-slate-700">{asset.name}</h4>
                      ...
                      <tbody>
                        {acctHoldings.holdings.map((h, hi) => (
                          <tr key={hi} className="border-b border-slate-100">
                            ...
                            <td className="py-2 text-slate-600 truncate max-w-[200px]">{h.name || "—"}</td>
                            ...
                            <td className="py-2">
                              <select
                                ...
                                value={h.asset_class || ""}
                                onChange={(e) => updateHolding(asset.name, hi, "asset_class", e.target.value)}
                              >
                                <option value="">—</option>
                                {Object.entries(ASSET_CLASS_LABELS).map(([k, v]) => (
                                  <option key={k} value={k}>{v}</option>
